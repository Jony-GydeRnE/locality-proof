...
By Laurentiu Rodina, Giuseppe Sellaroli, and Yán Nǐ (严你) or Jony

---

## ➡️ The paper

**[`trace(phi3)-FINAL-locality-proof.pdf`](trace(phi3)-FINAL-locality-proof.pdf)** — *All n proof: hidden zeros and locality on the n-gon* (9 pp, 20 July 2026).

This is the final Tr(φ³) locality result. Everything else in this folder is the trail that produced the original kill method which anchors the final kill: `paper/` holds the three-part development, `computations/` the machine verifications through n = 9, `notes/` the working record, `drafts/` the superseded write-ups.

The NLSM companion result lives in [`../NLSM locality/`](../NLSM%20locality/).
...
The hidden-zero theorem this work extends is due to **Laurentiu Rodina**
(arXiv:2406.04234). Any errors in the satellites here are the authors' own.
