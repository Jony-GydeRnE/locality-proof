...
- v2 (2026-05-30): rewrote with a rate-vector classification (since dropped --- that
  notation was internal bookkeeping and never made it into the notes).
- v3 (2026-05-31, morning): rewrote around the geometric P1/P2 theorem + stacking +
  multi-region rule, matching notes 20-22 directly. Geometric picture only.
- v4 (2026-05-31, later): added the partner-chord forcing logic (§5), the
  per-column choice (§6), the ~n/2 fan-crossing argument (§7), and a concrete
  worked surviving-multiset gallery (§8), matching notes 24-25. The substantive
  content the previous versions left out.
