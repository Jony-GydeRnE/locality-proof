...
\begin{abstract}
\noindent Part~II gave the geometric survival theorem on a $k$--zero $\Z^{(k)}$ (P1/P2: either a
chord crosses the enclosing chord, or the $(k{+}1)$--gon extends to a $(k{+}2)$--gon). The
vanishing of $B_n$ on
...
\emph{Step--$1$ recovered:} the geometric kill at the $2$--zero (Part~II Theorem~1) follows
automatically from this BP--graded D--subset decomposition.
...
The combination of Part~II (the geometric P1/P2 survival theorem on each $k$--zero) and the
present Step--$2$ relations gives, for each pair $(k,n)$ and each cyclic rotation, a closed
linear system on the multiset coefficients $a_M$. Three structural facts are immediate:
...
\end{itemize}
The empirical data through $n=9$ (Part~I, $\S5$) and the explicit small--$n$ relations recorded
in notes~26--27 are consistent with this conjecture. \textbf{An end--to--end rank computation
of the combined Part~II + Part~III system at $n=5,6,7$
(see \texttt{computations/full\_constraint\_system/}) confirms it directly: at each $n$ the
constraint matrix has rank $T(n)-1$, with kernel exactly $1$--dim and spanned by the
triangulation indicator.} A clean closed form for the kernel uniformly in $n$ is the next
target.
