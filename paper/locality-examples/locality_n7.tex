...
% ==============================================================
\section{Seven points: the cascade phenomenon}\label{sec:n7}
% ==============================================================

At $n = 7$ a genuinely new phenomenon appears. Of the Layer-$1$ failures
at $n=7$, \emph{one} cyclic orbit fails the admissibility criterion
(K0)--(K2) at \emph{every} zone. That orbit cannot be killed by a
single-term Laurent extraction at any one zone. Instead, the kill is
effected by a \emph{cascade}: a single multi-term relation at one zone,
combined with direct (Layer-$1$ or Layer-$2$) kills of the other terms
in that relation at a different zone, leaves the target coefficient
isolated. We work the cascade in full rigor for the exceptional orbit
below.

\subsection{Setup at $n=7$}

The $7$-gon has $14$ chords:
\[
\begin{array}{ll}
\text{\bf Short diagonals } (\text{class } 1,\ \text{cyclic distance } 2): &
  x_r = X_{r, r+2},\quad r = 1, \ldots, 7. \\
\text{\bf Long diagonals } (\text{class } 2,\ \text{cyclic distance } 3): &
  y_r = X_{r, r+3},\quad r = 1, \ldots, 7.
\end{array}
\]
Enclosed-vertex sets: $E(x_r) = \{r+1\}$, $E(y_r) = \{r+1, r+2\}$
(all indices mod $7$). Every chord has class at most $2$; no class-$3$
chord exists at $n=7$ since $\lfloor 7/2 \rfloor = 3$.

The ansatz \eqref{eq:ansatz} has
$\binom{14+3}{4} = 2380$ coefficients. Of these, $C_5 = 42$ are tree
triples, leaving $2338$ non-local coefficients.

\subsection{Zones in canonical form}

Each $\Zr_r$ imposes the four constraints
$c_{r,r+2} = c_{r,r+3} = c_{r,r+4} = c_{r,r+5} = 0$.
Applying the mesh identity \eqref{eq:mesh} and using $X_{aa+1} = 0$
yields, for $\Zr_1$ (by the same telescoping as at $n=5,6$):
\begin{align*}
x_2 &= y_1 - x_1, &\text{(non-bare, companion } y_1) \\
y_2 &= y_5 - x_1, &\text{(non-bare, companion } y_5) \\
y_6 &= x_6 - x_1, &\text{(non-bare, companion } x_6) \\
x_7 &= -\,x_1. &\text{(bare)}
\end{align*}
Special $X_\ast = x_1$; substituted chords $\{x_2, y_2, y_6, x_7\}$ are
precisely the four chords from the enclosed vertex $E(x_1) = \{2\}$;
free set
$\Fr_1 = \{x_3, x_4, x_5, x_6, y_1, y_3, y_4, y_5, y_7\}$ (9 chords,
matching $|\Fr_r| = (n-1)(n-4)/2 = 9$).

By cyclic rotation the remaining zones are read off identically. Each
zone $\Zr_r$ has three non-bare substitutions (each with a companion in
$\Fr_r$) and one bare substitution; nine free chords remain.

\subsection{Layer-1 coverage count}

A direct computer enumeration (attached: \texttt{n7\_locality.py})
gives:
\begin{itemize}\setlength\itemsep{0pt}
\item $2380$ total size-$4$ multisets;
\item $42$ tree triples (verified Catalan $C_5 = 42$);
\item $2338$ non-local multisets;
\item $2002$ of these satisfy $M \subseteq \Fr_r$ for some $r$
  (Layer-$1$ covered);
\item $336$ are Layer-$1$ failures.
\end{itemize}
The Layer-$1$ failures partition into exactly $48$ cyclic orbits under
$\sigma: r \mapsto r+1$, \emph{each of size $7$} (no orbit has any
nontrivial stabilizer).

\subsection{Admissibility of the 48 orbits}

For each of the $48$ orbit representatives, we check the admissibility
conditions (K0)--(K2) at each of the seven zones. The result is:
\begin{itemize}\setlength\itemsep{0pt}
\item \textbf{47 orbits} are admissible at at least one zone and are
  killed by direct single-term Laurent extraction at Layer $2$ or
  Layer~$4$ (using the companion-isolation mechanism of
  Lemma on companion isolation from \S\ref{sec:n7}).
\item \textbf{1 orbit} is admissible at \emph{no} zone. This is the
  \emph{exceptional orbit}:
\end{itemize}
\[
\text{Orbit 14:}\quad M_\star \;=\; \{x_1, x_2, x_4, x_6\}
\quad \text{(four short chords, cyclic composition $(4, 0)$).}
\]

We now prove $a_{M_\star} = 0$ in full rigor. The proof is a
\emph{cascade}: a four-term multi-term relation at $\Zr_5$ (order~$3$),
combined with single-term kills of the other four terms at $\Zr_4$.

\subsection{The exceptional orbit: $M_\star = \{x_1, x_2, x_4, x_6\}$}

\subsubsection*{Non-admissibility at every zone}

For each $r \in \{1, \ldots, 7\}$ one of (K0), (K1) fails:
\[
\begin{array}{c|c|c}
\text{Zone} & \text{special} = x_r & \text{bare} = x_{r-1}\ (\text{mod } 7) \\ \hline
\Zr_1 & x_1 \in M_\star & --\\
\Zr_2 & x_2 \in M_\star & --\\
\Zr_3 & -- & x_2 \in M_\star\\
\Zr_4 & x_4 \in M_\star & --\\
...
\end{array}
\]
So at the four zones $\{\Zr_1, \Zr_2, \Zr_4, \Zr_6\}$, (K0) fails;
at the three zones $\{\Zr_3, \Zr_5, \Zr_7\}$, (K1) fails. No zone is
admissible.

\subsubsection*{The key relation at $\Zr_5$}

At $\Zr_5$, the bare chord $x_4 \in M_\star$ means the canonical
companion-isolation argument cannot directly isolate $a_{M_\star}$.
However, we can still extract information at higher Laurent orders.
Substitutions at $\Zr_5$:
\[
x_6 = y_5 - x_5,\quad y_3 = x_3 - x_5,\quad y_6 = y_2 - x_5,\quad
x_4 = -x_5.
\]
The monomial $1/\prod(M_\star) = 1/(x_1 x_2 x_4 x_6)$ substitutes to
\[
\frac{1}{x_1 x_2 (-x_5)(y_5 - x_5)}
\;=\; \frac{1}{x_1 x_2}\cdot\frac{1}{x_5(x_5 - y_5)}.
\]
Expanding in $1/x_5$ (i.e., taking $x_5 \to \infty$ with $x_1, x_2, y_5$
fixed):
\[
\frac{1}{x_5(x_5 - y_5)}
\;=\; \frac{1}{x_5^2}\cdot\frac{1}{1 - y_5/x_5}
\;=\; \frac{1}{x_5^2} + \frac{y_5}{x_5^3} + \frac{y_5^2}{x_5^4} + \cdots.
\]
So at order $1/x_5^3$,
\[
\frac{1}{x_1 x_2 x_4 x_6}\bigg|_{\Zr_5,\ 1/x_5^3} \;=\; \frac{y_5}{x_1 x_2}.
\]
The rational function $y_5/(x_1 x_2)$ is what we call the
\emph{fingerprint}: a specific Laurent monomial in the nine free
variables of $\Fr_5$.

\paragraph{Which other multisets contribute the same fingerprint?}
A size-$4$ multiset $M'$ contributes to the coefficient of $y_5/(x_1 x_2)$
at order $1/x_5^3$ only if:
\begin{enumerate}\setlength\itemsep{0pt}
\item $x_1$ and $x_2$ appear each exactly once as free factors in $M'$
  (so the $1/(x_1 x_2)$ denominator structure arises directly).
\item The remaining two chord-factors of $M'$ produce $y_5$ in the
  Laurent numerator at the appropriate order. Since $y_5$ is the
  companion of $x_6$ at $\Zr_5$, this means the Laurent tail of $1/x_6$
  is the only source of $y_5$; therefore $M'$ must contain $x_6$.
\end{enumerate}

An exhaustive computer enumeration (scanning all $2380$ multisets,
filtering those containing $x_1, x_2$ each exactly once, and evaluating
the $y_5/(x_1 x_2)$ coefficient via symbolic Laurent expansion) yields
\emph{exactly six contributors}:
\begin{equation}\label{eq:relZ5}
\underbrace{a_{M_\star}}_{+1}
\;-\; \underbrace{a_{\{x_1,x_2,x_5,x_6\}}}_{-1}
\;+\; \underbrace{2\, a_{\{x_1,x_2,x_6,x_6\}}}_{+2}
\;+\; \underbrace{a_{\{x_1,x_2,x_6,y_3\}}}_{+1}
\;+\; \underbrace{a_{\{x_1,x_2,x_6,y_6\}}}_{+1}
\;-\; \underbrace{a_{\{x_1,x_2,x_6,y_5\}}}_{-1}
\;=\; 0.
\end{equation}

The signed coefficients are verified by direct computer-algebra
expansion; each is an integer $(\pm 1$ or $+2)$.

\subsubsection*{Using Layer-1 and Layer-2 at $\Zr_4$ to zero out five of
the six terms}

We show that five of the six coefficients in \eqref{eq:relZ5} vanish
via standard kills at a \emph{different} zone, $\Zr_4$. Setup:
$\Zr_4$ has special $x_4$, bare $x_3$, non-bare substitutions
$x_5 = y_4 - x_4$ (companion $y_4$), $y_2 = x_2 - x_4$ (companion $x_2$),
$y_5 = y_1 - x_4$ (companion $y_1$), and free set
$\Fr_4 = \{x_1, x_2, x_6, x_7, y_1, y_3, y_4, y_6, y_7\}$.

\paragraph{(a) Three Layer-$1$ kills at $\Zr_4$.} The three
multisets
\[
\{x_1, x_2, x_6, x_6\},\quad \{x_1, x_2, x_6, y_3\},\quad \{x_1, x_2, x_6, y_6\}
\]
are each contained in $\Fr_4$ (direct verification: $x_1, x_2, x_6, y_3, y_6 \in
\Fr_4$). Hence at order $1/x_4^0$ of $B|_{\Zr_4}$, each of these has a
linearly-independent fingerprint from $\Fr_4$ monomials, giving
\[
a_{\{x_1,x_2,x_6,x_6\}} \;=\; a_{\{x_1,x_2,x_6,y_3\}} \;=\; a_{\{x_1,x_2,x_6,y_6\}}
\;=\; 0. \qquad (\text{Layer-$1$ at } \Zr_4)
\]

\paragraph{(b) Two Layer-$2$ kills at $\Zr_4$.} We now kill
$a_{\{x_1,x_2,x_5,x_6\}}$ and $a_{\{x_1,x_2,x_6,y_5\}}$.

At $\Zr_4$, substitute $x_5 = y_4 - x_4$. The Laurent expansion is
$1/x_5 = -1/x_4 - y_4/x_4^2 - y_4^2/x_4^3 - \cdots$. Then
\[
\frac{1}{x_1 x_2 x_5 x_6} \;=\; \frac{1}{x_1 x_2 x_6}\cdot\frac{1}{x_5}
\;=\; -\frac{1}{x_1 x_2 x_6 x_4} - \frac{y_4}{x_1 x_2 x_6 x_4^2} - \cdots,
\]
so at order $1/x_4^2$ the contribution is $-y_4/(x_1 x_2 x_6)$. An
exhaustive enumeration over all size-$4$ multisets containing $x_1, x_2, x_6$
(each once) confirms that $\{x_1, x_2, x_5, x_6\}$ is the \emph{unique
source} of the fingerprint $y_4/(x_1 x_2 x_6)$ at order $1/x_4^2$
(the Laurent tail of $1/x_5$ is the only source of $y_4$ in $\Zr_4$
substitutions). Hence
\[
a_{\{x_1,x_2,x_5,x_6\}} \;=\; 0. \qquad (\text{Layer-$2$ at } \Zr_4)
\]

Similarly, substitute $y_5 = y_1 - x_4$ at $\Zr_4$:
$1/y_5 = -1/x_4 - y_1/x_4^2 - y_1^2/x_4^3 - \cdots$, and
$\{x_1, x_2, x_6, y_5\}$ is the unique source of fingerprint
$y_1/(x_1 x_2 x_6)$ at order $1/x_4^2$. Hence
\[
a_{\{x_1,x_2,x_6,y_5\}} \;=\; 0. \qquad (\text{Layer-$2$ at } \Zr_4)
\]

(The uniqueness of the source in each case is verified by the
computer-algebra enumeration; see \texttt{orbit14\_cascade\_close.py}.)

\subsubsection*{Closing the cascade}

Substituting all five kills back into \eqref{eq:relZ5}:
\[
a_{M_\star} \;-\; 0 \;+\; 2\!\cdot\!0 \;+\; 0 \;+\; 0 \;-\; 0
\;=\; 0 \quad\Longrightarrow\quad
\boxed{a_{\{x_1, x_2, x_4, x_6\}} \;=\; 0.}
\]
By cyclic symmetry ($\sigma$ on polygon vertices), the entire orbit of
size $7$ vanishes.

\begin{theorem}[Locality at seven points, orbit 14]\label{thm:n7orbit14}
For the ansatz $B$ satisfying $B|_{\Zr_r} = 0$ for all $r$, the
coefficient $a_M = 0$ for every $M$ in the cyclic orbit of
$\{x_1, x_2, x_4, x_6\}$.
\end{theorem}

\subsection{Completing $n=7$}

A complete proof of locality at $n=7$ requires verification of each of
the $48$ orbits. For the $47$ admissible orbits, the kill follows from
direct companion-isolation (Layer $2$ or $4$) at the admissible zone;
these can be handled uniformly by a generalization of the $n=5,6$
arguments. For the exceptional orbit $14$, the cascade above is the
complete rigorous proof. This leaves the general theorem:

\begin{theorem}[Locality at seven points; conditional]
\label{thm:loc7}
If (i) every one of the $47$ admissible orbits at $n=7$ is killed by
direct companion-isolation (Layer $2$ or $4$), and (ii) the exceptional
orbit~$14$ is killed by the cascade above, then every non-local
coefficient of $B$ vanishes at $n=7$. The surviving $42$ coefficients
are exactly the triangulations of the $7$-gon.
\end{theorem}

The first condition is a computational check across $47$ orbits, each
following the same single-term kill template as in $\S\ref{sec:n7}$; it
is not reproduced here in full. The second condition is
Theorem~\ref{thm:n7orbit14}.

\subsection{Consequences for the general theorem}

The $n=7$ finding shows:
\begin{enumerate}\setlength\itemsep{0pt}
\item The admissibility criterion (K0)--(K2) \emph{is not sufficient}
  to cover every non-local coefficient at general $n$. The
  exceptional orbit 14 at $n=7$ is the first counter-example.
\item The correct general statement is not ``some zone is admissible
  for every non-local $M$'' but rather ``some cascade of (Laurent
  multi-term relations + Layer-$1$/Layer-$2$ kills) reduces every
  non-local coefficient to zero.''
\item The cascade at $n=7$ orbit~$14$ uses just two layers of
  reasoning: one multi-term relation at $\Zr_5$ and single-term kills
  at $\Zr_4$. The structure is algorithmic: given the target
  coefficient, find a zone/order where the target appears in a
  multi-term relation whose other terms are all already-killable.
  Whether this cascade terminates at bounded depth for every $n$ is
  the open question.
\end{enumerate}
